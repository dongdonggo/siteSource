...
<h4>PDO object</h4>
<ul class="api_list">
<li id="api_list_pdo" class="on"><a href="doc.pdo.php">pdo</a></li>
</ul>
...
</div><div id="main_body"><h3>PDO Object</h3><p class="desc">Medoo是一个基于PDO的项目.你通过 $database->pdo, 能够使用更多的PDO方法, 预定义,事务 , 回滚 等. <br/>更多关于 PDO 的使用, 请阅读: http://php.net/manual/en/class.pdo.php.</p><h4>Transaction (事务)</h4><pre class="prettyprint linenums">$database->pdo->beginTransaction();$database->insert("account", [    "user_name" => "foo",    "email" => "foo@bar.com",    "age" => 25]);/* Commit the changes */$database->pdo->commit();/* Recognize mistake and roll back changes */$database->pdo->rollBack();</pre><h4>Prepare</h4><div class="note">有时候Medoo不能处理一些复杂的PDO时，你可以使用它来完成.</div><pre class="prettyprint linenums">$calories = 150;$colour = 'red';$sth = $database->pdo->prepare('SELECT name, colour, calories    FROM fruit    WHERE calories < :calories AND colour = :colour');$sth->bindParam(':calories', $calories, PDO::PARAM_INT);$sth->bindParam(':colour', $colour, PDO::PARAM_STR, 12);$sth->execute();</pre></div>></div><footer>
